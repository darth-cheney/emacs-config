...
Let's activate current line highlighting for all prog-modes
#+begin_src emacs-lisp
...
*** CSS and SASS
NOTE: scss-mode has been archived and is no longer compatible with modern flymake.
So we have removed it here.
*** Markdown
#+begin_src emacs-lisp
...
*** DAP Settings
Settings for the Debug Adapter Protocol
#+begin_src emacs-lisp
(use-package dape
...
#+end_src
*** Python
Ensure eglot is running for python
#+begin_src emacs-lisp
(add-hook 'python-ts-mode #'eglot-ensure)
#+end_src

Set auto mode for python files
#+begin_src emacs-lisp
...
#+end_src
** Company and Completions Settings :INACTIVE:
...
(defun nws/go-debug-exec ()
 ...
 (defun nws/go-debug-ghwo ()
   ...
   (let ((dape-config `(
           ...
           :request "launch"
           :mode "debug"
           :program ,(concat (projectile-project-root) "/tasks/cmd/ghwo/main.go")
           :showLog "true"
           :justMyCode "true"
   )))
   (dape dape-config)
...
Helper function to switch to proportional font
#+begin_src emacs-lisp
